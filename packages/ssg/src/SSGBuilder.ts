...
import { SSGConfig, SSGRoute } from './types.js';
...
export class SSGBuilder {
  ...
  private queue = new PrerenderQueue();
  ...
  async build(renderFn: (routePath: string) => Promise<string>): Promise<{ routesCount: number; errorsCount: number }> {
