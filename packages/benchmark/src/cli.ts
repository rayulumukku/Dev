...
import { BenchmarkOptions, ProjectScale, OutputFormat } from './types.js';
...
export async function runBenchmarkCLI(args: string[]) {
  ...
  let projectScale: ProjectScale = 'small';
  let runs = 10;
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--bundler' && args[i + 1]) {
      ...
      projectScale = args[i + 1] as ProjectScale;
      i++;
    } else if (arg === '--runs' && args[i + 1]) {
      runs = parseInt(args[i + 1], 10);
      i++;
    } else if (arg === '--output' && args[i + 1]) {
      outputFormat = args[i + 1] as OutputFormat;
      i++;
