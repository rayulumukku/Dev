...
import { Lexer, Parser, ASTVisitor } from '../../compiler/index.js';
function resolve(specifier, importer, resolver, projectRoot) {
...
export function jsxPlugin() {
    return {
        ...
        async transform(code, id) {
            ...
            const configCompiler = globalThis.__ray_cache_store?.config?.compiler || 'auto';
            ...
            if (configCompiler === 'ray') {
                const compiler = new (await import('../../compiler/index.js')).RayCompiler(globalThis.__ray_cache_store?.env || {});
                ...
                try {
                    ...
                    let rewritten = jsCode.replace(/(\b(?:import|export)\s+[\w\s*{},\$]+from\s+['"])([^'"]+)(['"])/g, (match, prefix, specifier, suffix) => {
                        try {
                    ...
                    rewritten = rewritten.replace(/(\bimport\s+['"])([^'"]+)(['"])/g, (match, prefix, specifier, suffix) => {
                        try {
                            let resolvedPath = specifier.startsWith('\0virtual:')
                                ? specifier
                                : resolve(specifier, id, this.resolver, this.projectRoot);
            ...
            visitor.resolveId = this.resolveId.bind(this);
            visitor.resolver = this.resolver;
            visitor.projectRoot = this.projectRoot;
            visitor.logger = this.logger;
            ...
            this.graph.updateDependencies(id, depIds, (depId) => {
                let depUrl = '';
