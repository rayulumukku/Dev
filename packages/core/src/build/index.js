...
export async function buildProject(options) {
    ...
    const sourcemapOption = options.sourcemap === 'true' || options.sourcemap === true
        ? true
    ...
    if (isLib) {
        ...
        for (const format of libConfig.formats) {
            ...
            const banner = configBuild.banner;
            const footer = configBuild.footer;
    ...
    if (mainAsset) {
        cleanHtml = cleanHtml.replace(new RegExp(`<script\\s+[^>]*type=["']module["'][^>]*src=["']\\/?${entryScript}["'][^>]*><\\/script>`, 'gi'), `<script type="module" src="/${mainAsset.file}"></script>`);
        if (mainAsset.css && mainAsset.css.length > 0) {
            const cssLink = mainAsset.css.map((cssFile) => `<link rel="stylesheet" href="/${cssFile}">`).join('\n');
    ...
    if (isSSG) {
        ...
        for (const route of routes) {
            ...
            const serializedState = JSON.stringify({ initialCount: 5 })
                ...
                .replace(/\u2028/g, '\\u2028')
                .replace(/\u2029/g, '\\u2029');
            const finalHtml = clientTemplate.replace('<div id="root"></div>', `<div id="root">${html}</div>\n<script>window.__RAY_DATA__ = ${serializedState};</script>`);
