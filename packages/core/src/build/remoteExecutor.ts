...
export class DistributedBuildExecutor {
  private ray: RayCore;
  private cloudClient: RayCloudClient;
  ...
  async runRemoteBuild(files: string[]): Promise<RemoteBuildSummary> {
    ...
    const compileTasks = files.map(async (file) => {
      ...
      // Compute CAS Key
      const key = this.cloudClient.computeCASKey(
        content,
        this.ray.config,
        lockfileContent,
        ['1.0.0']
      ...
      if (outputCode !== null) {
        // Cache Hit!
        cacheHits++;
        const node = this.ray.graph.registerModule(
          file,
          file,
          '/' + path.relative(this.ray.projectRoot, file).replace(/\\/g, '/')
        );
        node.status = 'clean';
      ...
      try {
        ...
        if (uploadSuccess) uploadedCount++;
      } catch {
  ...
  private tryReadLockfile(): string {
    const lockPaths = [
