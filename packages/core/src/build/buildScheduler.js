...
import { ChunkMerger } from './chunkMerger.js';
...
export class BuildScheduler {
     ...
     */
    topologicalGroups(files) {
        const fileSet = new Set(files);
        // Build a local in-degree map restricted to the given file set
        const inDegree = new Map();
        const adjacency = new Map(); // file → files that depend on it
        for (const file of files) {
            ...
            for (const dep of deps) {
                if (!fileSet.has(dep))
                    continue; // ignore out-of-set edges
     ...
     */
    async buildFiles(files, options = {}) {
        ...
        const result = { durationMs, compiledCount, results: perFile };
...
//# sourceMappingURL=buildScheduler.js.map