...
export class Parser {
    ...
    decorate(node) {
        ...
        const walk = (n) => {
            ...
            if (n.type === NodeType.Program || n.type === NodeType.BlockStatement) {
                ...
                if (Array.isArray(n.declarations))
                    n.declarations.forEach(walk);
            }
                ...
                if (Array.isArray(n.quasis))
                    n.quasis.forEach(walk);
    ...
    parseImportAttributes() {
        ...
        if ((this.match(TokenType.Keyword, 'with') || this.match(TokenType.Identifier, 'with') ||
    ...
    parseFunctionParams() {
        ...
        while (!this.match(TokenType.Punctuator, ')')) {
            ...
            // TS type annotation
            this.skipTypeAnnotation();
            let param = { type: NodeType.Identifier, name: paramName };
            // Default value
            if (this.eat(TokenType.Punctuator, '=')) {
                const defaultVal = this.parseAssignmentExpression();
    ...
    parsePrimary() {
        ...
        if (token.type === TokenType.Keyword) {
            switch (token.value) {
                ...
                case 'super':
                    this.advance();
                    return { type: NodeType.Identifier, name: 'super' };
    ...
    parseParenOrArrow() {
        ...
        try {
            ...
            while (!this.match(TokenType.Punctuator, ')') && !this.match(TokenType.EOF)) {
                ...
                this.skipTypeAnnotation();
                let paramNode = { type: NodeType.Identifier, name: param.value };
                if (this.eat(TokenType.Punctuator, '=')) {
                    const def = this.parseAssignmentExpression();
    ...
    // ── Function Expression ──────────────────────────────────────────────────
    parseFunctionExpression(isAsync) {
        this.consume(TokenType.Keyword, 'function');
        const isGenerator = !!this.eat(TokenType.Punctuator, '*');
        let id = null;
        if (this.match(TokenType.Identifier)) {
    ...
    parseObjectPattern() {
        ...
        while (!this.match(TokenType.Punctuator, '}') && !this.match(TokenType.EOF)) {
            ...
            const key = { type: NodeType.Identifier, name: keyTok.value };
            let value = key;
            if (this.eat(TokenType.Punctuator, ':')) {
    ...
    parseJSXElement() {
        ...
        const attributes = [];
        // Parse attributes
        while (this.peek().type !== TokenType.Punctuator ||
