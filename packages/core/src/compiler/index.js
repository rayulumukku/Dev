...
export class RayCompiler {
    ...
    compile(code, file, options = {}) {
        ...
        const countNodes = (n) => {
            ...
            if (n.consequent)
                countNodes(n.consequent);
            if (n.alternate)
                countNodes(n.alternate);
            if (n.init)
                countNodes(n.init);
            if (n.left)
                countNodes(n.left);
        ...
        // -----------------------------------------------------------------------
        let optimizedAst;
        if (_rustBridge && _rustBridge.isRustBackendActive?.()) {
            ...
            try {
                ...
                const optimizedJson = _rustBridge.optimizerOptimizeSync
                    ? _rustBridge.optimizerOptimizeSync(astJson)
                    : JSON.stringify(transformedAst); // safe fallback if sync API unavailable
                optimizedAst = JSON.parse(optimizedJson);
        ...
        if (_rustBridge && _rustBridge.isRustBackendActive?.()) {
            try {
                ...
                const resultJson = _rustBridge.codegenGenerateSync
                    ? _rustBridge.codegenGenerateSync(payload)
                    : null;
                if (resultJson) {
