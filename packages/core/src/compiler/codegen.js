...
export class CodeGenerator {
    ...
    generateWithSourceMap(node, sourceFile) {
        this.currentLine = 1;
        this.currentCol = 0;
        this.mappings = [];
    ...
    generate(node) {
        ...
        switch (node.type) {
            case NodeType.Program:
                return this.emit(node.body.map((child) => this.generate(child)).join(nl));
            // ── Imports ──────────────────────────────────────────────────────────
            case NodeType.ImportDeclaration: {
                ...
                const defaultSpec = specs.find((s) => s.type === NodeType.ImportDefaultSpecifier);
                const nsSpec = specs.find((s) => s.type === NodeType.ImportNamespaceSpecifier);
                const namedSpecs = specs.filter((s) => s.type === NodeType.ImportSpecifier);
                let importsPart = '';
                if (defaultSpec) {
                    importsPart += this.generate(defaultSpec.local);
                    if (nsSpec || namedSpecs.length > 0)
                        importsPart += `,${s}`;
                }
                if (nsSpec) {
                ...
                if (namedSpecs.length > 0) {
                    const specParts = namedSpecs.map((spec) => {
            ...
            case NodeType.ExportNamedDeclaration: {
                ...
                if (node.specifiers && node.specifiers.length > 0 && !node.declaration) {
                    const specs = node.specifiers.map((sp) => {
            ...
            case NodeType.VariableDeclaration: {
                const decls = node.declarations.map((d) => this.generate(d)).join(`,${s}`);
            ...
            case NodeType.FunctionDeclaration: {
                ...
                const params = node.params.map((p) => this.generate(p)).join(`,${s}`);
            ...
            case NodeType.ArrowFunctionExpression: {
                ...
                const params = node.params.length === 1 && node.params[0].type === NodeType.Identifier
                    ...
                    : `(${node.params.map((p) => this.generate(p)).join(`,${s}`)})`;
            ...
            case NodeType.ClassDeclaration: {
                ...
                if (node.id && node.id.name)
                    out += `${s}${this.generate(node.id)}`;
                if (node.superClass)
                    out += `${s}extends${s}${this.generate(node.superClass)}`;
                out += `${s}${this.generate(node.body)}`;
                return this.emit(out);
            }
            case NodeType.ClassBody: {
                const members = node.body.map((m) => this.generate(m)).join(nl);
                if (this.minified)
                    return this.emit(`{${members}}`);
            ...
            case NodeType.MethodDefinition: {
                ...
                const params = node.value?.params?.map((p) => this.generate(p)).join(`,${s}`) ?? '';
            ...
            case NodeType.PropertyDefinition: {
                ...
                if (node.value)
                    out += `${s}=${s}${this.generate(node.value)}`;
                out += ';';
                return this.emit(out);
            }
            ...
            case NodeType.BlockStatement: {
                const body = node.body.map((stmt) => this.generate(stmt)).join(nl);
                if (this.minified)
            ...
            case NodeType.SwitchStatement: {
                ...
                const cases = node.cases.map((c) => this.generate(c)).join(nl);
                if (this.minified)
                    return this.emit(`switch(${disc}){${cases}}`);
            ...
            case NodeType.SwitchCase: {
                ...
                const body = node.consequent.map((c) => this.generate(c)).join(nl);
                if (this.minified)
                    return this.emit(`${header}${body}`);
                const indented = body.split('\n').map((line) => '  ' + line).join('\n');
            ...
            case NodeType.CallExpression: {
                ...
                const args = node.arguments.map((arg) => this.generate(arg)).join(`,${s}`);
            ...
            case NodeType.OptionalCallExpression: {
                ...
                const args = node.arguments.map((a) => this.generate(a)).join(`,${s}`);
                return this.emit(`${callee}?.(${args})`);
            }
            case NodeType.SequenceExpression: {
                return this.emit(node.expressions.map((e) => this.generate(e)).join(`,${s}`));
            }
            ...
            case NodeType.ArrayExpression: {
                const elems = node.elements.map((e) => e ? this.generate(e) : '').join(`,${s}`);
            ...
            case NodeType.ObjectPattern: {
                const props = node.properties.map((p) => this.generate(p)).join(`,${s}`);
                return this.emit(`{${s}${props}${s}}`);
            }
    ...
    generateImportAttributes(attrs) {
        if (!attrs || attrs.length === 0)
            return '';
        const s = this.minified ? '' : ' ';
        const parts = attrs.map((a) => `${a.key.name}:${s}${a.value.raw}`).join(`,${s}`);
