...
export class RayCloudClient {
    ...
    computeCASKey(source, config, lockfile, pluginVersions) {
    ...
    isOnline() {
        ...
        if (globalThis.__ray_cloud_offline === true) {
            return false;
        }
        return true;
    }
    uploadArtifact(key, content) {
    ...
    syncOfflineQueue() {
        if (!this.isOnline() || !fs.existsSync(this.syncQueuePath))
            return 0;
        try {
            const queue = JSON.parse(fs.readFileSync(this.syncQueuePath, 'utf-8'));
            let syncedCount = 0;
