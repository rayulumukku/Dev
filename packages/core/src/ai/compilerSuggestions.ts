...
export class CompilerSuggestions {
  analyze(code: string, file: string): CompilerSuggestion[] {
    ...
    lines.forEach((line, idx) => {
      const lineNum = idx + 1;
      const col = (pattern: RegExp) => (line.search(pattern) ?? 0) + 1;
      ...
      if (/\w+={{\s*\w+:/.test(line)) {
        suggestions.push({
          ...
          message:
            'Inline object literal as JSX prop creates a new reference on every render. ' +
      ...
      if (/\.map\s*\(/.test(line) && /<\w/.test(line) && !(/key=/.test(line))) {
        suggestions.push({
          ...
          message:
            'JSX elements inside .map() require a unique "key" prop to help React reconcile the list.',
          severity: 'warning',
          line: lineNum,
          column: col(/\.map\s*\(/),
          file,
      ...
      if (/import\s*\(/.test(line) && !/\/\/ @error-boundary/.test(line)) {
        ...
        if (!/catch|ErrorBoundary|Suspense/.test(surroundingContext)) {
          suggestions.push({
            ...
            message:
              'Dynamic import() without a surrounding Suspense or ErrorBoundary. ' +
      ...
      if (ternaryMatches && ternaryMatches.length >= 3) {
        suggestions.push({
          ...
          message:
            'Deeply nested ternary expression (3+ levels). ' +
  ...
  httpHandler(): (req: any, res: any, getCode: (file: string) => string | null) => void {
    return (req: any, res: any, getCode: (file: string) => string | null) => {
