...
export class StaticPerformanceAnalyzer {
  ...
  analyze(graph: any, entryFiles: string[]): PerformanceAnalysis {
    ...
    const reports: ModulePerformanceReport[] = modules.map((mod: any) => {
      const code: string = mod.cachedOutput?.code ?? '';
  ...
  private bfsDepth(graph: any, startId: string, depthMap: Map<string, number>, depth = 0): void {
    const existing = depthMap.get(startId);
    if (existing !== undefined && existing >= depth) return;
    depthMap.set(startId, depth);

    const deps: Set<string> = graph.getDependencies?.(startId) ?? new Set();
