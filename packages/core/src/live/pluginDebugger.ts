...
export interface PluginTransformRecord {
  ...
  inputLength: number;
  outputLength: number;
  diff: string; // unified diff summary
  timestamp: number;
}

export class VisualPluginDebugger {
  private records: PluginTransformRecord[] = [];
  ...
  pluginStats(): Record<string, { totalMs: number; callCount: number; avgMs: number }> {
    const stats: Record<string, { totalMs: number; callCount: number }> = {};
    for (const r of this.records) {
      if (!stats[r.plugin]) stats[r.plugin] = { totalMs: 0, callCount: 0 };
  ...
  private generateDiffSummary(before: string, after: string): string {
