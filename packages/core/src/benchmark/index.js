 ...
 */
export async function runBenchmark(projectRoot, options) {
 ...
 */
export async function measurePerformance(projectRoot) {
...
export function comparePerformance(baseline, current) {
    ...
    const details = [];
    const checkLowerIsBetter = (name, base, curr, unit, absThreshold) => {
    ...
    const checkHigherIsBetter = (name, base, curr, unit) => {
        const diff = curr - base;
        let status = 'Stable';
        if (curr > base + 1.0) {
            status = 'Improved';
