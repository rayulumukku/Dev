...
export function parseSFC(code: string, filename: string): SFCDescriptor {
  ...
  let script: SFCDescriptor['script'] = undefined;
  if (scriptMatch) {
  ...
  let template: SFCDescriptor['template'] = undefined;
  if (templateMatch) {
  ...
  const styles: SFCDescriptor['styles'] = styleMatches.map((m) => {
    const attrs = m[1];
  ...
  const customBlocks: SFCDescriptor['customBlocks'] = customBlockMatches.map((m) => ({
    type: m[1],
    content: m[3].trim(),
  }));
